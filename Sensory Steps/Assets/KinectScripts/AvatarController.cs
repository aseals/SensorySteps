...
{	
	...
	// Whether the avatar's position is equal to the user's position, relative to this camera.
...
//		}
	...
	protected void MoveAvatar(Int64 UserID)
	{
		...
		if (!offsetCalibrated)
		{
			...
			xOffset = trans.x * moveRate; // !mirroredMovement ? trans.x * moveRate : -trans.x * moveRate;
			yOffset = trans.y * moveRate;
			zOffset = !mirroredMovement ? -trans.z * moveRate : trans.z * moveRate;
			...
			if(posRelativeToCamera)
			{
				...
				if(verticalMovement)
				{
					yRelToAvatar = (trans.y * moveRate - cameraPos.y) - (hipCenterPos - bodyRootPos).magnitude;
				}
				else
				{
